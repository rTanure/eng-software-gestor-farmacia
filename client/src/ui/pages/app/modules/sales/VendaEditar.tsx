...
export const VendaEditar = () => {
  const idSale = window.location.pathname.split("/").pop();

  const {data: sale} = useQuery(["sale", idSale], () => saleMdl.getById(idSale || ""), {
  ...
  const { data: clients } = useQuery("clients", () => clienteMdl.getAllClientes({name: ""}), {
    select: (data) =>
  ...
  const { data: prescriptions } = useQuery(["prescriptions", formValues.clientId], () => prescriptionMdl.getAllPrescriptions(), {
    select: (data) =>
      data.data.map((product) => ({ id: product.id, name: product.description })) || [],
                        ...
                        <Grid item sm={6}>
                          <TextField
                            ...
                            onChange={(e) =>
                              setFormValues({
                                ...formValues,
                                amount: isNaN(Number(e.target.value)) ? 0 : Number(e.target.value),
                              })
                            }
                            InputProps={{
                              startAdornment: (
                        ...
                        <Grid item sm={6}>
                          <TextField
                            ...
                            onChange={(e) =>
                              setFormValues({
                                ...formValues,
                                salePrice: isNaN(Number(e.target.value)) ? 0 : Number(e.target.value),
                              })
                            }
                            InputProps={{
                              startAdornment: (
