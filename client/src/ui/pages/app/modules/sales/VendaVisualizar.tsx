...
export const VendaVisualizar = () => {
  ...
  const {data: cliente} = useQuery(
    ["clientes", sale?.clientId],
    () => clienteMdl.getClienteById(sale?.clientId || ""),
    {
      select: data => data.data,
      enabled: !!sale?.clientId,
    }
  )

  const {data: stock} = useQuery(
    ["stock", sale?.productId],
    () => stockMdl.getById(sale?.productId || ""),
    {
      select: data => data.data,
  ...
  const {data: prescription} = useQuery(
    ["prescription", sale?.prescriptionId],
    () => prescriptionMdl.getPrescriptionById(sale?.prescriptionId || ""),
    {
      select: data => data.data,
      enabled: !!sale?.prescriptionId,
    }
  )
